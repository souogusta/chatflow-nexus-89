...
export default function Configuracoes() {
  const [users, setUsers] = useState(SELLERS.map((s, i) => ({ ...s, email: `${s.name.toLowerCase().replace(" ", ".")}@empresa.com`, role: i === 0 ? "Administrador" : "Vendedora", active: true })));
      ...
      <Tabs defaultValue="account">
        ...
        <TabsContent value="users" className="card-elevated p-6 mt-4">
          <div className="flex justify-between items-center mb-4">
            ...
            <Button className="bg-gradient-primary gap-2"><Plus className="w-4 h-4" /> Novo usuário</Button>
          </div>
              ...
              {users.map(u => (
                <tr key={u.id} className="border-b border-border/40">
                  ...
                  <td className="py-3 text-right">
                    ...
                    <Button size="icon" variant="ghost" className="text-destructive ml-1"><Trash2 className="w-3.5 h-3.5" /></Button>
                  </td>
                </tr>
              ))}
            </tbody>
